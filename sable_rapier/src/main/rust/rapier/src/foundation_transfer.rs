...
pub(super) fn dispatch(registry:&mut Registry,scene:i64,op:i32,ids:&[i64],values:&[f64])->Result<Vec<i64>,String> {
    ...
    if preview::retains(registry,scene)&&!matches!(op,0|3|7|8|11|13|20|22|30|35|40|42|44|49|50|56|57|58|74|76|77|79|83|84|85|90){return Err("staged interval retains native scene mutations".into());}
    ...
    match op {
        ...
        40..=49 | 57 | 58 | 74..=79 | 81..=85 => controlled::dispatch(registry,scene,op,ids,values),
...
pub extern "system" fn Java_dev_planetarysable_world_physics_FoundationNative_exchange<'a>(
    ...
    let result=std::panic::catch_unwind(std::panic::AssertUnwindSafe(||->Result<Vec<i64>,String>{
        ...
        // Only the bounded segmented motor command can carry more than 64 doubles.
        // Inspect its staged envelope before allocating the larger numeric payload.
        if n>20485||count>136{return Err("oversized typed foundation payload".into());}
        let mut ids=vec![0;n as usize];
        env.get_long_array_region(&identities,0,&mut ids).map_err(|e|e.to_string())?;
        let segmented=op==75||(op==63&&ids.get(1)==Some(&75));
        if (!segmented&&count>64)||(segmented&&n>if op==63{55}else{53}){
            return Err("oversized typed operation payload".into());
        }
